"""
Eval dataset generator — 2 unseen geometries, 100 frames each = 200 frames.
Seed 999 ensures entirely different geometry from the training set (seed 0).

Run:
    blender --background --python blender_depth_dataset_eval.py

Outputs to dataset_eval/ (separate from dataset_finetune/).
Use with run_da3_inference.py --dataset_dir dataset_eval to evaluate the
fine-tuned checkpoint on geometry it has never seen.
"""
...
_p.add_argument('--out_dir', default=os.path.join(_HERE, 'dataset_eval'))
_p.add_argument('--n_geoms', type=int, default=2)
_p.add_argument('--frames_per_geom', type=int, default=100)
_p.add_argument('--samples', type=int, default=32)
_p.add_argument('--seed', type=int, default=999)
