...
# 2026-08-02: 80.0 -> 10.0 N. Bench measurement on the physical scope: ~3 N of cable
# pull bends the tip through its full range in free space, ~10 N is the realistic
# maximum under load. The old 80 N rail was 8x the real ceiling -- it was never
# chosen against hardware, it was just high enough not to bind against the 27 N that
# the old joint stiffness (0.5) demanded. See the JOINT_STIFFNESS note below: the
# stiffness is the root cause and the rail merely accommodated it.
ACTUATOR_PULL_FORCE_LIMIT = 10.0
...
# 2026-08-02: RE-VERIFIED AND KEPT at 36000 after the stiffness/force-rail change above.
# The concern was that this was sized (~6x KP) against a droop about to shrink 9x, so it
# might now overshoot. It does not: 4-tendon diagnostic, both axes to full pull, gives
# 0.000 deg overshoot, no oscillation, and 98.4/98.6% of nominal bend -- matching the
# original spec. Under the env's outer PD shaper (i.e. real training conditions, not a
# step) peak tendon tension is ~7.8 N against the 10 N rail, so anti-windup never trips;
# it only engages on an artificial step command (2.7% of substeps). No change needed.
ACTUATOR_KI = 36000.0
...
#
# 2026-08-02: stiffness 0.5 -> JOINT_STIFFNESS (0.055), a 9.1x reduction. The 0.5
# value was never checked against hardware and made the tip ~9x too stiff: holding
# full bend cost 26.57 N of cable tension (measured free-space, tip only), against a
# bench measurement of ~3 N on the physical scope. Target derived directly:
#     k = T*r/theta = 3 N * 0.0032 m / 0.1745 rad = 0.055 N*m/rad
# The S-shape fix above is preserved, because it does not depend on the magnitude of
# k: the energy-minimising configuration under tendon tension is uniform bend
# (T*r = k*theta_i => all theta_i equal) for ANY k > 0. Only the restoring GRADIENT
# scales, so convergence to that equilibrium slows from tau ~= damping/stiffness
# = 0.035/0.5 ~= 0.07 s to 0.035/0.055 ~= 0.64 s -- still well inside a 6-7.5 s
# episode. Verified by re-running the same free-space diagnostic (drift + fold).
# Side effect: the P-control droop this stiffness caused (26.57/6000 = 4.4 mm out of
# 6.795 mm max pull, i.e. the "~35-45% of nominal" cap described above) falls to
# ~0.5 mm (~7%), so ACTUATOR_KI is no longer load-bearing for range -- it now only
# trims a small residual.
# If drift ever returns at this stiffness, do NOT raise it back: add joint
# frictionloss instead. Friction is dissipative and resists the slow creep at a
# CONSTANT tension cost, rather than one proportional to theta, so it does not
# re-inflate the force scale.
JOINT_STIFFNESS = 0.055
...
def add_disk(lines: list[str], disk_idx: int, indent: str) -> None:
    ...
    if disk_idx == 0:
        ...
        w(
            ...
            f'stiffness="{JOINT_STIFFNESS}" armature="0.00001"/>'
        )
        w(
