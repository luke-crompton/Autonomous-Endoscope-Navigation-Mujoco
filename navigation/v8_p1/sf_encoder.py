...
------------
  ...
  state (6,)         ->  MLP(64,64) ->  128-D feature
                                          └─ concatenate -> 640-D
                                          -> SF's shared actor-critic head

The 640-D vector goes into SF's built-in policy/value heads (linear layers).
...
class _DepthCNN(nn.Module):
    """
    Convolutional encoder for single-channel depth images (currently 54×96).
    ...
    without code changes — which is why the 2026-08-04 move from a square
    64×64 obs to 16:9 54×96 (Change C) needed no edit here at all.

    Input:  (B, 1, H, W) float32    — normalised depth image, channel-first.
    Output: (B, 512) float32        — flat feature vector.

    Spatial sizes at 54×96 input (H, W tracked separately — the stack is no
    longer square):
      Conv1 (k=3, s=2): 54 → 26   |  96 → 47
      Conv2 (k=3, s=2): 26 → 12   |  47 → 23
      Conv3 (k=3, s=1): 12 → 10   |  23 → 21
      Flatten: 10×21×64 = 13440 → Linear → 512

    For reference, the pre-2026-08-04 square obs gave 13×13×64 = 10816. A
    checkpoint's `encoder.depth_cnn.net.1.weight` therefore tells you which
    obs geometry it was trained on.
    """
...
class ScopeColonEncoder(Encoder):
    ...
    def __init__(self, cfg, obs_space) -> None:
        ...
        # MLP for the low-dimensional state vector. Width is read from the obs
        # space, NOT hardcoded: this was `STATE_DIM = 8` with a "must match
        # scope_colon_env.py STATE_OBS_DIM" comment, which is precisely the kind
        # of pairing that drifts. It nearly did on 2026-08-04 when the state went
        # 8D -> 6D (ten_x_n/ten_z_n removed). Now it cannot -- same approach the
        # depth CNN already used for its input resolution.
    ...
    def forward(self, obs_dict: dict) -> torch.Tensor:
        ...
        state_features = self.state_mlp(state)    # (B, 128)

        return torch.cat([cnn_features, state_features], dim=-1)  # (B, 640)
