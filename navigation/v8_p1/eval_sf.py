...
Usage:
    ...
    # Rendering defaults to osmesa (CPU) -- the right choice in WSL2, where eval
    # runs. On the native rig, GPU rendering is faster:
    python eval_sf.py --gl egl --episodes 3 --seeds 80 --workers 24

    NOTE: an inherited MUJOCO_GL env var OVERRIDES the default (precedence is
    --gl flag > MUJOCO_GL > DEFAULT_GL). Don't prefix this with MUJOCO_GL=egl.
"""
...
# (render backend is chosen by _select_gl_backend() below, not by a shell prefix)
os.environ.setdefault("OMP_NUM_THREADS", "1")
os.environ.setdefault("MKL_NUM_THREADS", "1")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")

GL_BACKENDS = ("egl", "osmesa", "glfw")
DEFAULT_GL  = "osmesa"   # CPU rendering. Eval runs in WSL2, where egl is ~7x slower
                         # (GPU paravirtualisation makes every depth render a
                         # high-latency round-trip, so CPU and GPU both idle).
                         # On the native rig, pass --gl egl.
...
def _select_gl_backend() -> str:
    ...
    osmesa is CPU-only and is the DEFAULT, because eval runs in WSL2. There it is
    both the *stable* and the *faster* choice (~24 ms/env-step vs ~179 ms under
    egl, whose GPU paravirtualization also throws EGLError on context teardown).
    On the native rig, egl wins -- pass --gl egl there.
    """
...
N_EPISODES = 3          # episodes per seed. 5 -> 3 on 2026-08-04: an env step covers
                        # 40 ms of task rather than 10 ms since Change F, so each episode
                        # is ~4x more wall-clock, and 3 x 80 seeds = 240 episodes is still
                        # a wide enough sample to rank seeds. Override with --episodes.
N_SEEDS    = 80
DEFAULT_WORKERS = 24
    ...
    with csv_path.open("w", newline="") as f:
        ...
        for rank, s in enumerate(summaries, start=1):
            ...
            row = {
                ...
                # "HxW", not the raw tuple -- a tuple stringifies with a comma
                # inside it and the csv writer would quote it into one awkward cell.
                "depth_res": f"{args.depth_res[0]}x{args.depth_res[1]}",
...
def _depth_res_arg(s: str) -> tuple[int, int]:
    """--depth_res accepts "HxW" (e.g. 54x96) or a single int meaning square.

    Ordering is HEIGHT x WIDTH, matching numpy's .shape and the env's
    (DEFAULT_DEPTH_RES_H, DEFAULT_DEPTH_RES_W) -- NOT the WxH convention image
    tools use. The default 54x96 is 16:9 landscape, so if you pass something
    taller than it is wide you have almost certainly swapped them.
    """
    s = s.strip().lower()
    if "x" in s:
        h, w = s.split("x", 1)
...
_W_cfg          = None
_W_depth_res: tuple[int, int] = DEPTH_RES
_W_max_steps: int = MAX_STEPS


def _worker_init(
    ckpt_path_str: str, depth_res: tuple[int, int], max_steps: int, experiment: str,
) -> None:
    ...
    obs_space     = _spaces.Dict({
        "depth": _spaces.Box(
            low=0.0,
            high=1.0,
            shape=(1, depth_res[0], depth_res[1]),
            dtype=_np.float32,
        ),
        "state": _spaces.Box(
            low=-_np.inf,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate v8_p1 checkpoint")
    ...
    parser.add_argument("--episodes",   type=int, default=N_EPISODES,
                        help=f"episodes per seed (default: {N_EPISODES})")
    ...
    parser.add_argument("--gl", choices=GL_BACKENDS, default=GL_BACKEND,
                        help=f"MuJoCo render backend (default: {DEFAULT_GL}, CPU-only — "
                             f"the fast choice under WSL2; pass egl on the native rig)")
    parser.add_argument("--depth_res", type=_depth_res_arg, default=DEPTH_RES,
                        help=f"depth obs size as HxW, or one int for square "
                             f"(default: {DEPTH_RES[0]}x{DEPTH_RES[1]}). Must match "
                             f"what the checkpoint was TRAINED at or the encoder's "
                             f"first Linear will shape-mismatch on load.")
