...
"""
Interactive keyboard test for v8_p1 flexible shaft dynamics.
...
# one recursive call per nested <body>. v8_p1 nests 44 shaft links (fused into
# 22 pairs) + 25 tip disks, deep enough to overflow Windows' default 1 MB thread
# stack (STATUS_STACK_OVERFLOW — a bare exit-127 with NO Python traceback).
# Building the model on a thread with a bigger stack avoids this without
# changing the scene itself. See build_collision_scene.py's SHAFT_N_LINKS note
# for the depth budget: ~77 levels at 50 links, and 100 links hard-fails.
MODEL_BUILD_STACK_SIZE = 64 * 1024 * 1024   # 64 MB

PHYSICS_PER_STEP = 20           # MuJoCo substeps per rendered frame (0.5 ms each = 10 ms/frame).
                                # NOT the env's control rate: scope_colon_env.py uses 80 substeps
                                # (40 ms = 25 Hz) since Change F. This is deliberately the finer of
                                # the two -- it is an interactive feel constant, and TENDON_DECAY /
                                # ROLLER_OMEGA_CMD below are tuned per FRAME against it. Raising it
                                # to 80 to "match" the env would advance 40 ms of sim per keypress
                                # frame and make manual driving jumpy.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Manual keyboard test — v8_p1")
