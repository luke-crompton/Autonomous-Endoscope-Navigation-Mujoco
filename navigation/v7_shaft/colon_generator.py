...
VISUAL_N_AXIAL = 500
VISUAL_N_RADIAL = 36
COLLISION_N_AXIAL = 260
COLLISION_N_RADIAL = 30
DISTAL_STRAIGHT_EXTENSION_M = 0.20
DISTAL_VISUAL_SPACING_M = 0.005

TASK_LENGTH_M = 0.50
CTRL_SPACING_M = 0.020
BEND_COUNT_MIN = 4
BEND_COUNT_MAX_EXCLUSIVE = 6      # 4-5 bends
STRAIGHT_LEN_RANGE_M = (0.010, 0.060)
ENTRANCE_STRAIGHT_RANGE_M = (0.10, 0.13)  # first straight segment only: must clear
                                           # SCOPE_TIP_AT_S (95mm, build_collision_scene.py)
                                           # + margin so the rigid disk0->tip insertion
                                           # span always sits inside straight tube
BEND_ANGLE_RANGE_RAD = (np.radians(80), np.radians(150))
BEND_RADIUS_RANGE_M = (0.032, 0.048)
...
def random_ctrl_points(seed: int) -> tuple[np.ndarray, float, int]:
    ...
    for _ in range(100):
        ...
        entrance_len = rng.uniform(*ENTRANCE_STRAIGHT_RANGE_M)
        remaining_total = (
            TASK_LENGTH_M - entrance_len - float(sum(bend_poly_lengths))
        )
        rest_lengths = _sample_bounded_lengths(
            rng,
            n_sections=n_bends,
            total_length=remaining_total,
            bounds=STRAIGHT_LEN_RANGE_M,
        )
        if rest_lengths is not None and remaining_total >= 0.0:
            straight_lengths = np.concatenate([[entrance_len], rest_lengths])
            break
    else:
    ...
    for seg in range(n_bends + 1):
        ...
        if seg < n_bends:
            ...
            # Loop-avoidance bias: of the two arc directions (+-u2), prefer the
            # one that curves AWAY from the path travelled so far. Chained
            # random-axis bends have no memory of earlier segments, so total
            # turning routinely exceeds 360 deg and folds back through the
            # entrance (the exact geometry behind seeds 43/34's self-
            # intersecting colons). This doesn't guarantee avoidance -- that's
            # what the min-self-distance rejection below is for -- it just
            # lowers how often the rejection loop needs to fire.
            away = pos - origin
            away -= away.dot(direction) * direction
            if np.linalg.norm(away) > 1e-9 and u2.dot(away) < 0.0:
                u2 = -u2
    ...
    ctrl = np.array(pts)
    radius = rng.uniform(0.018, 0.022)
...
def build_tube(
    ...
    # Arc-length parameter: fixes visual/collision mesh mismatch.
    # linspace(0,1) normalises each mesh to its own total length, so the 900mm
    # visual centreline (task + 200mm tail) and the 700mm collision centreline
    # map the same physical position to different t_p values → haustra rings
    # are out of phase by up to ~180°. Dividing by the fixed TASK_LENGTH_M
    # gives the same t_p at any physical position across all mesh resolutions.
    _ds = np.linalg.norm(np.diff(centreline, axis=0), axis=1)
    t_p = np.concatenate([[0.0], np.cumsum(_ds)]) / TASK_LENGTH_M
...
def centreline_min_self_distance(
    centreline: np.ndarray, arc_exclude_m: float,
) -> float:
    """Closest approach (metres) between two centreline points that are more
    than `arc_exclude_m` apart in ARC LENGTH -- i.e. a genuine fold-back
    between distant sections, not local tube thickness/adjacent samples on
    the same bend.

    centreline_min_curvature_radius() above only catches a tube pinching
    itself WITHIN a single bend (local curvature). It has no way to see two
    unrelated sections of a 320-750deg-total-turning path (4-5 bends of
    80-150deg each) swinging back through each other in 3D -- chained
    random-axis bends have no memory of where the path has already been.
    That's a distinct failure: found on 6/80 eval seeds of v7_shaft_v4,
    2 of them (seeds 43, 34) with a fold-back within the first ~15% of the
    colon that plugged the lumen right past the entrance and made the
    episode un-survivable regardless of policy quality (18-step ejection,
    contact count ~10x a healthy step). See generate_colon().
    """
    ds = np.linalg.norm(np.diff(centreline, axis=0), axis=1)
    s = np.concatenate([[0.0], np.cumsum(ds)])
    dist = np.linalg.norm(centreline[:, None, :] - centreline[None, :, :], axis=-1)
    arc_gap = np.abs(s[:, None] - s[None, :])
    dist = np.where(arc_gap > arc_exclude_m, dist, np.inf)
...
SELF_INTERSECT_ARC_EXCLUDE_M = 0.060
# Any two centreline points >6cm apart in arc length must stay this many
# tube-diameters apart, or the walls fold back and interpenetrate. 1.0 is the
# literal "at least 2*radius" no-touching boundary; the tiny excess is fp/
# quantisation slack, not a deliberately loose margin -- the natural
# population already clears far higher margins (min ~1.2x across 74/80 eval
# seeds), so this rejects only the handful of genuinely self-intersecting
# layouts (6/80) without materially changing the task distribution.
SELF_INTERSECT_MARGIN = 1.02
...
def generate_colon(seed: int) -> dict:
    ...
    satisfies is_valid(margin=MIN_CURVATURE_MARGIN), its arc length is within
    MAX_COLON_ARC_M, AND it does not fold back and self-intersect anywhere
    along its length (SELF_INTERSECT_MARGIN -- see centreline_min_self_distance),
    or for up to MAX_GENERATION_ATTEMPTS attempts (after which the closest
    valid-looking candidate is used).
    ...
    for _ in range(MAX_GENERATION_ATTEMPTS):
        ...
        self_dist = centreline_min_self_distance(task_centreline, SELF_INTERSECT_ARC_EXCLUDE_M)
        self_ratio = self_dist / (SELF_INTERSECT_MARGIN * 2.0 * max(radius, 1e-12))
        self_deficit = max(0.0, 1.0 - self_ratio)
        score = 10.0 * curve_deficit + length_excess + 10.0 * self_deficit
        if score < best_score:
            best_score = score
            best_candidate = (ctrl, radius, n_haustra, task_centreline)
        if curve_deficit == 0.0 and length_excess == 0.0 and self_deficit == 0.0:
