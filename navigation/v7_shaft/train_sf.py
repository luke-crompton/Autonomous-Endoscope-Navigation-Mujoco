"""
Sample Factory APPO training for ScopeColonEnv — v7_shaft flexible-shaft physics.

v7_shaft replaces v6_dr's kinematic mocap-driven base advance with a real
physical shaft (universal-joint chain) fed by an entrance roller/capstan
mechanism; domain randomisation on tendon gains, dead-zones, and lag is
retained, but the synthetic base advance-gain DR is removed since real
contact friction now provides that variability physically.
...
OUTPUT
------
  runs_sf/v7_shaft_v3/checkpoint_p0/
"""
...
class ScopeStatsObserver(AlgoObserver):
    ...
    def _maybe_print(self, env_steps: int, runner=None) -> None:
        ...
        print(
            f"\n── [SF v6]  {env_steps:>10,} / {self.total_steps:,}  ({pct:.1f}%)"
