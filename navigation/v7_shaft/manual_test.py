...
MAX_PULL         = scope_gen.MAX_PULL          # 0.024 m
...
TENDON_STEP  = 0.001            # 1 mm per event  (MAX_PULL=24 mm → 24 taps for full curl)
...
def main() -> None:
    ...
    print(
        ...
        f"  MAX_PULL = {MAX_PULL*1000:.0f} mm -> full curl in {int(MAX_PULL/TENDON_STEP)} taps.\n"
    )
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=_key_cb) as viewer:
        ...
        while viewer.is_running() and not _quit_flag:
            ...
            cmd_x = float(np.clip(cmd_x + dx, -MAX_PULL, MAX_PULL))
            cmd_z = float(np.clip(cmd_z + dz, -MAX_PULL, MAX_PULL))
            ...
            if frame % 60 == 0:
                ...
                pct_x = cmd_x / MAX_PULL * 100
                pct_z = cmd_z / MAX_PULL * 100
