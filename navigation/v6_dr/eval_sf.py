"""
Evaluate a Sample Factory V6 700 mm checkpoint on each training seed.

Usage:
    python eval_sf.py
    python eval_sf.py --checkpoint runs_sf/v6_dr_700V2/checkpoint_p0
    python eval_sf.py --checkpoint runs_sf/v6_dr_700V2/checkpoint_p0/checkpoint_XXX.pth
...
DEFAULT_CHECKPOINT = str(Path(TRAIN_DIR) / EXPERIMENT / "checkpoint_p0")
...
def _resolve_checkpoint(path_arg: str | None) -> Path:
...
def _list_checkpoints(train_dir: str, experiment: str) -> list[Path]:
    ckpt_dir = Path(train_dir) / experiment / "checkpoint_p0"
    return sorted(
...
def run_episode(env: ScopeColonEnv, actor_critic, cfg) -> dict:
    ...
    completed = info.get("termination_reason") == "goal"
    raw_final_progress = float(info.get("progress", 0.0))
...
def _seed_to_summary(seed: int, results: list[dict], n_episodes: int) -> dict:
    ...
    raw_progs = np.array([r["raw_final_progress"] for r in results])
    ...
    welds_mm = np.array([r["weld_lag_m"] * 1000.0 for r in results])
    near_miss_99 = sum(
        1 for r in results
    ...
    return {
        ...
        "raw_progress_mean": float(raw_progs.mean()),
        "raw_progress_min":  float(raw_progs.min()),
        "raw_progress_max":  float(raw_progs.max()),
        ...
        "weld_lag_mm_mean": float(welds_mm.mean()),
...
def _write_summary_csv(
    ...
    fieldnames = [
        ...
        "progress_max_pct",
        "raw_progress_mean",
        "raw_progress_mean_pct",
        "raw_progress_min",
        "raw_progress_min_pct",
        "raw_progress_max",
        "raw_progress_max_pct",
        "goal_gap_mm_mean",
        "goal_gap_mm_min",
    ...
    with csv_path.open("w", newline="") as f:
        ...
        for rank, s in enumerate(summaries, start=1):
            ...
            row = {
                ...
                "raw_progress_mean": f"{s['raw_progress_mean']:.6f}",
                "raw_progress_mean_pct": f"{s['raw_progress_mean'] * 100.0:.3f}",
                "raw_progress_min": f"{s['raw_progress_min']:.6f}",
                "raw_progress_min_pct": f"{s['raw_progress_min'] * 100.0:.3f}",
                "raw_progress_max": f"{s['raw_progress_max']:.6f}",
                "raw_progress_max_pct": f"{s['raw_progress_max'] * 100.0:.3f}",
                ...
                "weld_lag_mm_mean": f"{s['weld_lag_mm_mean']:.3f}",
            }
...
def _worker_init(ckpt_path_str: str, depth_res: int, max_steps: int) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate V6 700 mm checkpoint")
    ...
    if args.list_checkpoints:
        ckpts = _list_checkpoints(TRAIN_DIR, EXPERIMENT)
        if not ckpts:
            print("No checkpoints found.")
        else:
            print(f"Checkpoints in runs_sf/{EXPERIMENT}/checkpoint_p0/:")
            for p in ckpts:
                print(f"  {p.name}")
        return
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(str(ckpt_path), args.depth_res, args.max_steps),
    ) as ex:
