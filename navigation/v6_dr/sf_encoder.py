...
------------
  ...
  state (10,)        ->  MLP(64,64) ->  128-D feature
                                          └─ concatenate -> 576-D
...
class ScopeColonEncoder(Encoder):
    ...
    STATE_DIM = 10   # must match scope_colon_env.py STATE_OBS_DIM (cmd_x, cmd_y, progress placeholder, force_norm, net_fx, net_fz, last_action[3], tip_contact)
    CNN_OUT   = 512
    MLP_OUT   = 128
    ...
    def __init__(self, cfg, obs_space) -> None:
        ...
        # MLP for the 10-D low-dimensional state vector. The progress slot is
        # a constant placeholder so the policy does not receive privileged sim
        # progress information.
        self.state_mlp = nn.Sequential(
            nn.Linear(self.STATE_DIM, 64),   # STATE_DIM=10
            nn.ReLU(),
